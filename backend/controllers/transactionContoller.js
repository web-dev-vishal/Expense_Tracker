...
export const getTransactions = async (req, res) => {
    try {
        ...
        const sevenDayAgo = _sevenDaysAgo.toISOString().split("T")[0];

        const { df, dt, s } = req.query;

        const { userId } = req.body.user;

        const startDate = new Date(df || sevenDayAgo);
...
export const getDeshboardInformation = async (req, res) => {
    try {
        const { userId } = req.body.user;
...
export const addTransaction = async (req, res) => {
    try {
        ...
        if (!(description || source || amount)) {
            return res.status(403).json({ status: "Failed", message: "Provide all the Required Fields" });
        }
...
export const transferMoneyToAccount = async (req, res) => {
    try {
        ...
        // Transfer from account from ( one acc to another acc )
        await pool.query({
            text: `UPDATE tblaccount SET account_balance = account_balance - $1, updatedat = CURRENT_TIMESTAMP WHERE id = $2`,
